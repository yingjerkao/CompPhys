...
# Writes the bin averages to the file res.dat, writes a message to 'log.log'
 ...
 f=open("read.in","r")
    ll=parse(Int64,readline(f))      # Lattice size (length)
    tt=parse(Float64,readline(f))    # Temperature (J=1)
    hh=parse(Float64,readline(f))    # Magnetic field
    istp=parse(Int64,readline(f))    # MC steps for equilibration
    mstp=parse(Int64,readline(f))    # MC steps per bin
